...
// Mock data storage
const MOCK_TRACES = new Map<string, TraceExecution>([
  [
    'exec-001',
    {
      ...
      root_decisions: [
        ...
        },
        {
          ...
          decision_context:
            'Executed 156 unit tests, 42 integration tests, and 8 end-to-end tests. Coverage: 87% (↑3% from last run). All critical path tests passed. 1 flaky test identified in retry logic. Performance tests show 95th percentile response time: 234ms.',
          confidence_score: 91,
          ...
          alternatives_considered: [
            {
              ...
              rejection_reason:
                'Issues are moderate and can be fixed. Regeneration would be costly.',
            },
          ],
        },
      ],
...
export async function GET(
  ...
  // First check if we have hardcoded mock data for this ID
  let trace = MOCK_TRACES.get(executionId);

  // If not found, generate a response for any execution ID (mock data)
  if (!trace) {
    // Generate a realistic trace for any execution ID
    trace = {
      ...
      status: ['success', 'warning', 'pending'].length > 0 ? 'success' : 'warning',
      root_decisions: [
        {
